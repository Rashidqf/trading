    ...
    await Promise.all(accounts.map(async (account) => {
      ...
      if (exitFrom) {
        ...
        const exited = await Promise.all(parent.childrens.map(async (children) => {
          if (children.account.toString() === account._id.toString()) {
            ...
            req.body.tradeId = single.tradeId;
            return single;
          }
          return children;
        }));
      ...
      if (exitFrom && ammount !== 0) {
        const log = await genLog('Pending', 'Trade opened');
...
export const createOrderPartialExit =
      ...
      await Promise.all(
        accounts.map(async (account) => {
          ...
          if (ammount !== 0) {
            ...
            // Delete the trade immediately after pushing it
            await db.remove({
              table: Order,
              key: { _id: order._id },
            });
  ...
  // export const SinglePartialExit = ({ db, ws }) => async (req, res) => {
  //   try {
  //     const isValid = Object.keys(req.body).every((key) =>
  //       createAllowed.has(key)
  //     );
  ...
  //         if (exitFrom) {
  //           const parentOrder = await Order.findById(exitFrom).populate({
  //             path: "childrens",
  //           });
  
  //           if (parentOrder) {
  //             // Update each child order and create new orders
  //             const createdOrders = await Promise.all(
  //               parentOrder.childrens.map(async (children) => {
  //                 const newPer = Number(children.percentage) - Number(percentage);
  //                 const ammount = await calcAmmount(req.body.ammount, percentage); // Calculate amount for each order
  ...
  //                       percentage: newPer,
  //                       ammount,
  //                       tradeId: children.tradeId, // Update with tradeId from child
  //                     },
  //                   },
  //                 });
  
  //                 // Create a new order for each child
  //                 const log = await genLog("Pending", "trade opened");
  ...
  //                     ...req.body,
  //                     ammount,
  //                     tradeId: children.tradeId, // Use tradeId from child
  //                     logs: [log],
  //                     account: account._id,
  //                     populate,
  //                   },
  ...
  //                 // Return newOrder to push later
  //                 return newOrder;
  //               })
  //             );
  
  //             // Push all created orders to the orders array
  //             orders.push(...createdOrders);
  
  //             parentOrder.childrens = parentOrder.childrens.map((child, index) => ({
  //               ...child,
  //               tradeId: createdOrders[index].tradeId,
  //             }));
  ...
  //           // Optionally, you can remove the created order if needed
  //           // await db.remove({
  //           //   table: Order,
  //           //   key: { _id: newOrder._id },
  //           // });
  //         }
  //       })
  //     );
  ...
  //     if (orders.length === 0)
  //       return res.status(400).send({ message: "No orders created" });
  ...
  //       await ws.emit("account", response.data);
  //     } catch (err) {
  //       console.log("Error in sending data to BOT_SERVER_URL", err);
  //       res.status(500).send({ message: "Failed to send data to external service" });
  //     }
  //   } catch (err) {
  //     console.log("Error in SinglePartialExit handler", err);
  //     return res
  //       .status(500)
  //       .send({ message: err.message || "Internal Server Error" });
  //   }
    ...
    try {
      ...
      const order = await Promise.all(
        orders.childrens.map(async (order) => {
          ...
          req.body.logs = [
            ...order.logs,
            await genLog("Updated", "Trade Updated"),
          ];
    ...
    try {
      ...
      const childs = await Promise.all(
        trade.childrens.map(async (children) => {
          const logs = [
            ...children.logs,
            await genLog("Closed", "Trade Closed"),
          ];
          const updated = await db.update({
            table: Order,
      ...
      if (idsArray) {
        ...
        for (const orderId of ids) {
          ...
          if (!order)
            return res
              .status(404)
              .send({ message: `Trade with ID ${orderId} not found` });
          order.status = status;
          ...
          ws.emit("order", {
            order,
            message: `Trade has been ${status} from account ${order.account.email}`,
          });
        }
        return res.status(200).send({ message: "Trade updated successfully" });
      } else {
        ...
        if (!order) return res.status(404).send({ message: "Trade not found" });
        order.status = status;
        order.orderCreated = orderCreated;
        order.openPrice = openPrice;
        order.tradeId = tradeId;
