...
def place_order(instances: list, data: dict) -> str:
    ...
    def place_new_trade(bot, result):
        ...
        success = bot.order(
            ...
            result[13], result[14], status_url, result[18]
        )
        if success:
...
def place_Trade(instances: list, data: dict) -> str:
    ...
    def trade_to_order(bot, result, myamount):
        success = bot.tradetoOrder(
            result[1], myamount, result[2], result[3], result[4], result[5], 
            result[6], result[7], result[8], result[9], result[10], result[11], 
            result[12], result[13], result[14], status_url, result[18]
        )
        if success:
            print("Order to Trade executed successfully")
        else:
            print("Order to Trade failed")
        results.append(success)
    ...
    def exit_trade(bot, result, order, op):
        ...
        success = bot.ordercancel(
            result[2], result[1], Exit, result[4], status_url, result[18], 
    ...
    if isinstance(data.get("order"), list):
        for order in data.get("order"):
            ...
            print(toOrder)
            if toOrder:
                t = threading.Thread(target=trade_to_order, args=(bot, result, myamount))
                threads.append(t)
            elif result[0]:
