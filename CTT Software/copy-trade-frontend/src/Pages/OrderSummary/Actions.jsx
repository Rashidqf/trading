...
export default function Actions({ orderData, ordertype }) {
  ...
  const handleExit = async () => {
    try {
      setLoading(true);
      const action = await updateTrade(orderData.id, {
        status: "Closed",
        exit: "Exit",
        placeOrder: false,
      });
  ...
  const onsubmit = async (data) => {
    ...
    try {
      const action = await createOrder(newOrderData);
                  ...
                  {tradeType === "order" ? (
                    <div>
                      ...
                      <input
                        ...
                        type="number"
                        {...register("orderLevel")}
                        className="w-28 outline-none border text-center border-grey-400 px-1"
                      />
                    </div>
                  ) : (
                    <></>
                      ...
                      <div className="mt-2 flex gap-5">
                        <div>
                          ...
                          <input
                            type="number"
                            {...register("pointsAway")}
                            className="outline-none border border-grey-400 px-1 w-28 text-center"
                          />
                        </div>
                        <div>
                          <p className="text-xs font-thin tracking-wide">
