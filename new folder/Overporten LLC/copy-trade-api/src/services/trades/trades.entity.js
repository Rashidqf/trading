...
import { calAmountbyU, genLog } from "../order/order.helper";
  ...
  async (req, res) => {
    try {
      ...
      try {
        ...
        return res.status(200).send({ orders, parentOrder });
      } catch (err) {
        console.log(err);
      }
    } catch (err) {
  ...
  async (req, res) => {
    try {
      ...
      try {
        ...
        return res.status(200).send({ updated, orders: orders.docs[0] });
      } catch (err) {
        console.log(err);
      }
    } catch (err) {
